...
if (subsample) {
  ...
  f_circsum <- file.path(data_path,paste0(exp, "_circ_data_seed",seed,".RDS"))

} else {
  data_samp <- data
  
  f_circsum <- file.path(data_path,paste0(exp, "_circ_data-alldata.RDS"))
}
```
...
# ct <- circ_summary %>% dplyr::select(-p) %>% 
#                         rename(`$n$` = n, 
#                                `$\\bar{\\theta}$` = theta_bar, 
#                                `$\\bar{r}$` = r_bar,
#                                `$Vm$` = vm, 
#                                `$v$` = v, 
#                                `$\\mu_{MLE}$` = mle_mu, 
#                                `$\\mu CI_{lo}$` = bs_mu_ci_l, 
#                               `$\\mu CI_{hi}$` = bs_mu_ci_h, 
#                                `$\\kappa_{MLE}$` = mle_kappa, 
#                                `$Rt$` = rt, 
#                                `$p adj.$` = p_adjust) %>%
#                             as.data.frame() 
...
if (use_all) {
  f_mu_bs <- file.path(data_path,paste0(exp, "_boot_mu_kappa-all.RDS"))
} else {
  f_mu_bs <- file.path(data_path,paste0(exp, "_boot_mu_kappa_seed",seed,".RDS"))
} 
...
if (file.exists(f_mu_bs)) {
  ...
  if (use_all) {
    save_name = file.path(data_path,paste0(exp, "_boot_mu_kappa-all"))
    mu_bs_df.samp <- mu_bs_df
  } else {
    save_name = file.path(data_path,paste0(exp, "_boot_mu_kappa_seed",seed))
  }
  write.csv(mu_bs_df, file = paste0(save_name, ".csv"), row.names = F)
...
prediction_for_roc_curve <- predict(shPeak_rf_classifier,mu_bs_valid[,-3],type="prob")

# Use pretty colours:
pretty_colours <- RColorBrewer::brewer.pal(n=8,name="Spectral")
# Specify the different classes 
classes <- levels(mu_bs_valid$modulation)
# For each class
for (i in 1:length(levels(mu_bs_valid$modulation))) {
 # Define which observations belong to class[i]
 true_values <- ifelse(mu_bs_valid[,3]==classes[i],1,0)
...
timing adaptation to stabilize, estimate mean and concentration parameters for successively
larger cycle windows:
...
```{r 3d-plots-render, eval=FALSE, include=FALSE}
# Show interactive plots for angular means over each cumulative 5-cycles:
pltypal <- RColorBrewer::brewer.pal(n=8,name="Spectral")
  if (exp == "exp_1") {
  pltypal <- c("#F46D43","#FFA054")
  }
pltypal <- setNames(pltypal, unique(by_cycle_summary$modulation))

plts <- list()
for (cond in unique(by_cycle_summary$condition)) {
  pltdat <- by_cycle_summary[by_cycle_summary$condition==cond,] %>%
      mutate(text = paste0("cycles cum.: ", n, "\n", 
                           "mu: ", theta_bar, "\n",
                           "kappa: ", r_bar))

  
  
  plt <- plot_ly(data =pltdat,
                x= ~n, y=~theta_bar, z=~r_bar, 
                type="scatter3d", mode="markers", 
                color=~modulation,
                colors=pltypal,
                size = 2) %>%
        layout(title = cond,
               scene = list(xaxis = list(title = 'elapsed cycles', autorange = "reversed"),
               yaxis = list(title = 'mu', tickvals = seq(0, 2*pi, length.out = 5),
                            ticktext = c("0","pi/2","pi","3pi/2","2pi")),
               zaxis = list(title = 'kappa')))
  
  plts <- append(plts,list(plt))
  
  print(plt)
  htmlwidgets::saveWidget(plt, file=file.path(figures_path,paste0(exp, '_cycle_traj_',cond,'_1600.html')))
  # this saves a widget as an html
  #rmarkdown::pandoc_convert("C:/Users/Ava Kiai/OneDrive - Johann Wolfgang Goethe Universität/#Bat Lab/_Projects/colony_jam/Experiment_2/4-figures_tables/figures/exp_2_cycle_traj_random masker.html", output = 'C:/Users/Ava Kiai/OneDrive - Johann Wolfgang Goethe Universität/#Bat Lab/_Projects/colony_jam/Experiment_2/4-figures_tables/figures/output.pdf')
...
### References
Some references for circular stats can be found from the 
[NCSS website](https://ncss-wpengine.netdna-ssl.com/wp-content/themes/ncss/pdf/Procedures/NCSS/Circular_Data_Analysis.pdf), as well as the documentation for the [Matlab](https://www.jstatsoft.org/article/view/v031i10) library for 
circular stats, and articles written on [Bayesian modelling of circular data](https://journals.sagepub.com/doi/pdf/10.1177/1471082X10000301). 

There is also the textbook, [*Statistical analysis of circular data*](https://books.google.de/books?id=wGPj3EoFdJwC&printsec=frontcover#v=onepage&q&f=false), as well as:
