...
packages <- c("circular", "CircStats", "CircSpaceTime", "sjPlot", "janitor", "broom", "knitr",  "kableExtra", "car", "MASS", "emmeans", "rcompanion", "pscl", "caret", "MLeval", "viridis", "scales","ggh4x","ggside", "ggdist", "ggridges","tidyverse","here")
...
*NB: The code below generates subfigures used in Figs. 2 & 3*
...
*NB: The code below generates subfigures used in Figs. S2 & S4*
...
if (subsample) {
  ...
  set.seed(seed) #.Random.seed if full random desired
...
*NB: The code below generates subfigures used in Fig. S3*
...
data were highly overdispersed. 
For example, for the 8 Hz context, overdispersion was:
...
*NB: The code below generates subfigures used in Figs. 2 & 3*
```{r}
#@editor requested: show points
n_detailed <- data %>% group_by(modulation, condition, session, group) %>% summarise(n = n()) #%>%
              #group_by(modulation, condition, group) %>% summarise(mean = mean(n))
p.new <- ggplot(n_detailed, aes(x = condition)) +
    geom_point(aes(y = n, 
                   group = modulation,
                   color = modulation), 
               alpha = 0.3,
...
p.new <- p.new %>% recolor()
p.new
ggsave(file.path(figures_path,paste0(exp, '_n_calls_pred_editor.pdf') ), p.new, width = 5, height = 3)
ggsave(file.path(figures_path,paste0(exp, '_n_calls_pred_editor.png') ), p.new, width = 5, height = 3)
```
...
*NB: The code below generates a subfigures and graphical components used in Fig. 4*
...
*NB: The code below generates subfigures used in Fig. 4*
```{r lda-auc}
...
*NB: The code below generates subfigures used in Fig. S5*
```{r}
#@editor requested: show points
n_overlap_detailed <- overlap_calls %>% group_by(modulation, condition, group, session) %>% summarise(n = n())
p.new <- ggplot(n_overlap_detailed, aes(x = condition)) +
    geom_point(aes(y = n, 
                   group = modulation,
                   color = modulation), 
               alpha = 0.3,
               position = position_dodge2(width = 0.4, padding = 0.3), 
               #color = "darkslategray", 
               size = 3) +
    geom_pointrange(data = ovlp_preds, 
                    aes(y = fit, ymin = fit-se.fit, ymax = fit+se.fit,
                        group = modulation,
                        color = modulation),
...
p.new <- p.new %>% recolor()
p.new
ggsave(file.path(figures_path,paste0(exp, '_overlap_pred_editor.pdf') ), p.new, width = 5, height = 4)
ggsave(file.path(figures_path,paste0(exp, '_overlap_pred_editor.png') ), p.new, width = 5, height = 4)
```
